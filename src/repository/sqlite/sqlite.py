...
class Repository():
    ...
    async def set_user(self, user_id: int, language: str = 'ru'):
        self.cursor.execute("""INSERT OR REPLACE INTO users (id, lang) VALUES (?, ?)""",
                            (user_id, language))
        self.conn.commit()
    ...
    async def set_token(self, access_token : str, user_id : int):
        self.cursor.execute("""INSERT OR REPLACE INTO users (id, access_token) VALUES (?, ?)""",
